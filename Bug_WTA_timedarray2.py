...
dv/dt = (0.04*v**2+5*v+140-u+r1*I+(foodcount+1)*1.1*g_a_syn*(E_syn-v))/ms: 1
du/dt=a*(b*v-u)/ms: 1
...
sensor_agg_eqs = '''
# equations for neurons
dv/dt = (0.04*v**2+5*v+140-u+r1*I+(foodcount+1)*1.1*g_a_syn_a*(E_syn-v))/ms: 1
du/dt=a*(b*v-u)/ms: 1
...
ML = StateMonitor(sl, ('v', 'r1', 'I', 'g_a_syn'), record=True)
ML2 = StateMonitor(sl2, ('v', 'r1', 'I', 'g_a_syn_a'), record=True)
MR = StateMonitor(group1, ('r','Iconct'), record=True)
MR2 = StateMonitor(group2, ('r','Iconct'), record=True)
MLL = StateMonitor(sbl, ('v', 'r1', 'I'), record=True)
MLL2 = StateMonitor(sbl2, ('v', 'r1', 'I'), record=True)
