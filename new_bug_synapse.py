...
#A1=TimedArray((0,1),dt=4000*ms)
#A2=TimedArray((1,0),dt=4000*ms)
stimlist=numpy.zeros(4000)
pulselist=[100,150,1000,2140,2170,3220]
...
A1=TimedArray(stimlist*mV/mV,dt=1*ms)
p1=A1
# post
stimlist=numpy.zeros(4000)
pulselist=[110, 120, 1055, 2145, 2180, 3250]
stimlist[pulselist]=1 
...
A2=TimedArray(stimlist*mV/mV,dt=1*ms)
p2=A2

sensor_eqs = '''
# equations for neurons
dv/dt = (0.04*v**2+5*v+140-u+r1*I+mag1*p1(t)+mag2*p2(t) +g_a_syn*(E_syn-v))/ms: 1
du/dt=a*(b*v-u)/ms: 1
...
sensor_agg_eqs = '''
# equations for neurons
dv/dt = (0.04*v**2+5*v+140-u+r1*I+mag1*p1(t)+mag2*p2(t) +g_a_syn_a*(E_syn-v))/ms: 1
du/dt=a*(b*v-u)/ms: 1
...
taupre2 = taupost2 = 40*ms
wmax = 2*.8
Apre = 0.01
Apost = -Apre*taupre/taupost*1
Apre2 = 0.01
Apost2 = -Apre2*taupre2/taupost2*1
...
syn_rr=Synapses(sr, sbl, clock=Clock(0.2*ms), model='''
		...
		g_a_syn += we
        apre += Apre
             	we = clip(we+apost, 0, wmax)
        ''',
        on_post='''
		apost += Apost
		we = clip(we+apre, 0, wmax)
		''',method='euler')
...
syn_rr2=Synapses(sr2, sbl2, clock=Clock(0.2*ms), model='''
                g_a_syn_a_post = g_a_a: 1 (summed)
                we2 : 1
                dapre2/dt = -apre2/taupre2 : 1 (clock-driven)
                dapost2/dt = -apost2/taupost2 : 1 (clock-driven)
		...
		g_a_syn_a += we2
        apre2 += Apre2
             	we2 = clip(we2+apost2, 0, wmax)
        ''',
        on_post='''
		apost2 += Apost2
		we2 = clip(we2+apre2, 0, wmax)
		''',method='euler')
